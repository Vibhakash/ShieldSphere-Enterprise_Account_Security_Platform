...
| Session management and location map | Lists active sessions for the authenticated user, including IP, approximate location, browser, last-used time, and expiry. Users can revoke one session or all sessions; mapped public locations reveal structured session details on hover. | FastAPI, SQLAlchemy async ORM, React Leaflet, APScheduler |
| Device management and location map | Stores device identifiers and parsed browser/OS/device data from successful logins. Users can view approximate device locations, trust/untrust, or remove their own devices. Long browser and device details wrap within responsive cards. | FastAPI, SQLAlchemy, React Leaflet, device fingerprint derivation |
| Login history and locations | Records successful and failed login attempts with IP, device, outcome, location, anomaly score, and simulation flag. Deployments use an optional MaxMind GeoLite City web-service fallback when the local MMDB file is unavailable. | PostgreSQL, GeoIP2, MaxMind GeoLite2 City/web service |
...
| User behavior analytics | Builds a baseline from at least two successful sign-ins using login hours, devices, countries, ASNs, and login frequency. It exposes per-device activity, approximate location, login/logout timing, session status, and anomaly signals. | PostgreSQL, SQLAlchemy, APScheduler, UBA engine |
| Executive incident reports | Aggregates real account activity over a requested period, generates a structured executive summary, persists the report, and produces a readable color-accented PDF security report. | PostgreSQL, Groq SDK, FastAPI, ReportLab |
| Audit trail, deletion, and GDPR export | Returns paginated audit records, lets a user delete individual records or clear their own audit history, and generates an Excel export of profile, login history, threats, alerts, devices, sessions, and audit data. | PostgreSQL, FastAPI, openpyxl |
...
| Health and verification | Exposes a readiness health endpoint that validates both PostgreSQL and Redis, and includes scripts for migrations, tests, compilation, Alembic validation, Docker image builds, and opt-in PostgreSQL/Redis integration tests. | FastAPI, Alembic, unittest, Docker, PowerShell |

## Deployment and production experience

- **Local and deployed routing:** The local frontend uses the local API; Vercel builds use `VITE_API_ORIGIN`, `VITE_API_BASE_URL`, and `VITE_SIMULATOR_WS_URL` to target the deployed Render API.
- **Cross-site authentication:** Local development uses `SameSite=Lax` cookies. A Vercel frontend and Render API use secure HTTP-only `SameSite=None` cookies through `COOKIE_SAMESITE=none`; CORS should then be limited to the exact frontend origin.
- **Cloud data services:** Render can use Neon PostgreSQL and a TLS-enabled hosted Redis URL. The health endpoint becomes ready only after both PostgreSQL and Redis are reachable.
- **Location enrichment:** The local GeoLite2 City MMDB is intentionally excluded from Git. Set `MAXMIND_ACCOUNT_ID` and `MAXMIND_LICENSE_KEY` in Render to enable the deployed GeoLite City fallback; locations are approximate and recorded for new sign-ins.
- **Readable security views:** Sessions, devices, and map hover panels wrap long browser/user-agent strings within responsive cards and tooltips instead of overflowing their boundaries.
